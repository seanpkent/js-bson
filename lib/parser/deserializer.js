...
function deserializeObject(buffer, index, options, isArray) {
  ...
  const promoteArrayBuffers = options['promoteArrayBuffers'] == null ? false : options['promoteArrayBuffers'];
  ...
  while (!done) {
    ...
    if (elementType === constants.BSON_DATA_STRING) {
      ...
      if (buffer['slice'] != null) {
        ...
        if ((promoteBuffers || promoteArrayBuffers) && promoteValues) {
          if (subType === constants.BSON_BINARY_SUBTYPE_UINT8_ARRAY) {
            object[name] = new Uint8Array(buffer.buffer.slice(index, index + binarySize));
          } else {
            object[name] = promoteArrayBuffers
              ? buffer.buffer.slice(index, index + binarySize)
        ...
        if ((promoteBuffers || promoteArrayBuffers) && promoteValues) {
          if (subType === constants.BSON_BINARY_SUBTYPE_UINT8_ARRAY) {
            ...
            object[name] = promoteArrayBuffers ? _buffer.buffer : _buffer;
          }
        } else {
