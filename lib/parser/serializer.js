...
function serializeTypedArray(buffer, key, value, index, isArray) {
  // Write the type
  buffer[index++] = constants.BSON_DATA_BINARY;
  // Number of written bytes
  const numberOfWrittenBytes = !isArray
  ...
  buffer[index++] = (size >> 24) & 0xff;
  // Write the default subtype
  buffer[index++] = constants.BSON_BINARY_SUBTYPE_UINT8_ARRAY;
  // Copy the content form the binary field to the buffer
  value.copy(buffer, index, 0, size);
  // Adjust the index
  index = index + size;
  ...
  if (Array.isArray(object)) {
    ...
    for (let i = 0; i < object.length; i++) {
      ...
      if (type === 'string') {
        ...
        index = serializeBuffer(buffer, key, Buffer.from(value), index)
      } else if (value instanceof Uint8Array) {
        index = serializeTypedArray(buffer, key, Buffer.from(value), index)
